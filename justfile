...
clean_rootfs: _unmount_rootfs
  rm -f {{_rootfs_built_sentinel}} {{_sysroot_img}}
...
_create_rootfs_image size="4GiB":
  rm -f {{_sysroot_img}}
  {{_fallocate}} -l {{size}} {{_sysroot_img}}
  sudo {{_mkfs_ext4}} {{_sysroot_img}}
  touch {{_create_rootfs_sentinel}}

[group('rootfs')]
[working-directory: 'boot']
clean_rootfs_image: _unmount_rootfs
  rm -f  {{_create_rootfs_sentinel}} {{_sysroot_img}}
